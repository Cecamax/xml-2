        ...
        foreach($choosen->channel->item as $news){
            if($counter < 3){
                echo "<div style='background: #eee; color: #000; padding: 10px; margin-bottom: 10px'>";
                    echo "<h2 style='color: #2e7038'>" . $news->title . "</h2>";
                    echo "<p>" . $news->descripton . "</p>";
                    echo "<a style='text-decoration: none; color: #2e7038' href='" . 
                    $news->link . "'target='_blank'>Read more</a>";
                    echo "</div>";
            }
        }
}
